...
namespace TestGame.Huds
{
    public class MonsterInfo : Hud
    {
        ...
        {
            if(g.pageGame.getHudManager().activeObject is Monster)
            {
                
                Vector2 cords = GetPos(g);

                Monster monster = (Monster)g.pageGame.getHudManager().activeObject;
                ...
                new Sprite(Textures.icons, new Rectangle(32 * 0, 32 * 0, 32, 32)).Draw(1090, 560, 30);
                ...
                Drawing.DrawText(MonsterHandler.getSpeedLevel(monster.Speed), 1140, 590);
                if(monster.armor > 0)
                {
